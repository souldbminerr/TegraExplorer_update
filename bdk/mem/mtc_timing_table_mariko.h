 ...
 */
#pragma once
#include "mtc_timing_table_common.h"
...
#define DECLARE_ARRAY_AND_STRUCT_MEMBERS(NAME, FOREACH_HANDLER) \
    union { \
        struct { FOREACH_HANDLER(DECLARE_STRUCT_MEMBER_HANDLER) } NAME; \
        uint32_t NAME##_arr[sizeof(struct { FOREACH_HANDLER(DECLARE_STRUCT_MEMBER_HANDLER) }) / sizeof(uint32_t)]; \
    }

typedef struct _MarikoTiming {
    uint32_t emc_rc;
    uint32_t emc_rfc;
    uint32_t emc_rfcpb;
...
typedef struct _EmcDvfsTimingTable {
    uint32_t rev;
    char dvfs_ver[60];
    uint32_t rate_khz;
    uint32_t min_volt;
    ...
    union {
        ...
        uint32_t     burst_regs_arr[sizeof(MarikoTiming)/sizeof(uint32_t)];
    };
    ...
    union {
        ...
        uint32_t burst_perch_regs_arr[8];
    };

    union {
        MarikoTiming shadow_regs_ca_train;
        uint32_t     shadow_regs_ca_train_arr[sizeof(MarikoTiming)/sizeof(uint32_t)];
    };

    union {
        MarikoTiming shadow_regs_rdwr_train;
        uint32_t     shadow_regs_rdwr_train_arr[sizeof(MarikoTiming)/sizeof(uint32_t)];
    };
    ...
    union {
        ...
        uint32_t trim_perch_regs_arr[10];
    };
    ...
    union {
        ...
        uint32_t vref_perch_regs_arr[4];
    };
    ...
    union {
        ...
        uint32_t training_mod_regs_arr[20];
    };
